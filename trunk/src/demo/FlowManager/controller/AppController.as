package demo.FlowManager.controller {
	...
	public class AppController extends FlowSection {
		...
		function AppController() {
			...
			listen();
			initFlowManager();
			new FrameDelay(display);
			// wait one frame, otherwise we get a flickering of assets on the stage
		}
		...
		protected function initFlowManager() : void {
			FM.setDownloadDirectory("");
			// use current dir

			var rule : FlowRule;

			// after the intro, go to section 1
			rule = new FlowRule(AppSettings.SECTION_INTRO, FlowOptions.START_END, FlowOptions.ANY, proceedToSection1);
			FM.addRule(rule);

			// do not hide with sibling sections
			rule = new FlowRule(null, FlowOptions.STOP, FlowOptions.DISTANT | FlowOptions.SIBLING, doNotHide);
			FM.addRuleForSections(rule, [AppSettings.SECTION1, AppSettings.SECTION2, AppSettings.SECTION3, AppSettings.SECTION4]);

			rule = new FlowRule(null, FlowOptions.START, FlowOptions.ANY, moveSection);
			FM.addRuleForSections(rule, [AppSettings.SECTION1, AppSettings.SECTION2, AppSettings.SECTION3, AppSettings.SECTION4]);
		}

		protected function display() : void {
			visible = true;
			// necessary because this is a FlowSection as well
